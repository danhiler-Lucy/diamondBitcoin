...
activeMainPage.getData = function () {
    activeProductManager.getFilterProducts('fromPrice=1&colorType=white&model=RB&color=I,J,K&clarity=FL,IF,VVS1,VS1,VS2&symmetry=EX&polish=EX');
}

activeMainPage.getGuestData = function () {
    $.ajax({
        url: activeMainTool.serverSrc + 'guest/products',
        type: 'get',
        dataType: 'json',
...
activeMainPage.getSearchData = function () {
    ...
    if (inputValue.length > 4) {
        ...
        $('.mainContentWrapper').css('margin-top', '-240px');
    }
